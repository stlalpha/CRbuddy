...
- **CodeRabbit bot matching is exact, not substring**: a comment author counts as the bot only if its login equals the configured bot login (default `coderabbitai`) or that login plus `[bot]`, case-insensitively. This was a real bug once (`strings.Contains`); don't reintroduce it.
