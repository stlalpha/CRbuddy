...
- **Test `gh`-shelling code with a fake `Runner`**, not real `gh` calls. See `internal/ghclient/fake_runner_test.go` for the pattern (`RunFunc func(...) ([]byte, error)`), and `prs_test.go`/`threads_test.go` for how it's used to test JSON parsing, pagination, and error paths without touching the network. `internal/ghrepo`'s tests, and `internal/mcpserver`'s test of `resolveRepo` (which wraps `ghrepo.Detect`), are the exception — they exercise real local `git` in a temp dir (no network, no GitHub), which is fine.
